      ...
      <div className="bg-[#FFF9F9] text-on-surface border border-[#F7B6B0] rounded-3xl max-w-4xl w-full shadow-2xl relative max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleCreateProductSubmit} className="space-y-4 text-xs">
            ...
            <div className="lg:col-span-5 bg-white p-4 rounded-2xl border border-[#F7C5C0] shadow-sm space-y-4 sticky top-0">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[0, 1, 2, 3].map((idx) => {
                  ...
                  const isSlotUploading = !!imageUploadProgress[idx];
                  const slotPercent = imageUploadProgress[idx] || 0;
                  return (
                    <div 
                      key={idx}
          ...
          <div className="pt-3 border-t border-[#F7C5C0] flex items-center justify-end space-x-2.5">
            ...
            <button
              type="submit"
              disabled={videoUploadLoading || Object.keys(imageUploadProgress).length > 0}
              className={`px-5 py-2.5 rounded-xl shadow-md border border-black/20 flex items-center gap-1.5 font-bold text-[11px] uppercase tracking-wider transition-all cursor-pointer ${
                (videoUploadLoading || Object.keys(imageUploadProgress).length > 0)
            ...
            >
              <span className={`material-symbols-outlined text-sm ${(videoUploadLoading || Object.keys(imageUploadProgress).length > 0) ? 'animate-spin text-[#B78946]' : ''}`}>
                {(videoUploadLoading || Object.keys(imageUploadProgress).length > 0) ? 'progress_activity' : 'verified'}
              </span>
              <span>
                {videoUploadLoading 
