...
export default function AdminHeader({ onExitAdmin }) {
  ...
  return (
    <>
      <header className="w-full h-12 bg-[#FCDAD7] border-b border-[#F7B6B0] px-3 md:px-4 flex items-center justify-between fixed top-0 left-0 right-0 z-50 shadow-md">
        ...
        {/* FAR RIGHT: Dynamic Role Badge + Change Password + Exit Button */}
        <div className="ml-auto flex items-center gap-1.5 md:gap-2">
          {isReadOnly ? (
            <span className="bg-amber-900 text-amber-100 text-[10px] px-2 py-0.5 rounded-md border border-amber-700 font-bold flex items-center gap-1 shadow-xs" title="Secondary Admin: Read & Export Only">
              <span className="material-symbols-outlined text-[12px] text-amber-200">visibility</span>
              <span className="hidden md:inline">Read + Export (Agency)</span>
              <span className="md:hidden">Agency</span>
            </span>
          ) : (
            <span className="bg-black text-[#FCDAD7] text-[10px] px-2 py-0.5 rounded-md border border-black/20 font-bold flex items-center gap-1 shadow-xs" title="Primary Admin: Full Management Access">
          ...
          <button
            ...
            className="px-2 py-1 bg-white hover:bg-amber-50 text-black hover:text-amber-900 text-[11px] font-bold rounded-lg border border-black/20 shadow-xs flex items-center gap-1 transition-all active:scale-95 cursor-pointer"
          ...
          {onExitAdmin && (
            <button
              type="button"
              onClick={onExitAdmin}
              className="px-2 py-1 bg-white hover:bg-red-50 text-red-700 hover:text-red-800 text-[11px] font-bold rounded-lg border border-red-200 shadow-xs flex items-center gap-1 transition-all active:scale-95 cursor-pointer"
      ...
      {/* Authenticated Admin Change Password Modal */}
